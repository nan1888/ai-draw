...
function isImageItem(value) {
    return Boolean(value &&
        typeof value === 'object' &&
        (typeof value.url === 'string' || typeof value.b64_json === 'string'));
}
...
function taskStatus(response) {
    return normalizedStatus(response.data?.status ??
...
export class NewApiAsyncImageAdapter {
    ...
    static async isConfigured(workspaceRoot) {
        ...
        return Boolean(((process.env.NEWAPI_BASE_URL || process.env.AI_CANVAS_IMAGE_BASE_URL || settings?.baseUrl) &&
    ...
    async generateImage(request) {
        ...
        const submit = await this.postJson(config, '/images/async/generations', payload);
    ...
    async editImage(request) {
        ...
        const submit = await this.postForm(config, '/images/async/edits', form);
        const taskId = getTaskId(submit);
        await this.emit({
            stage: 'submitted',
            taskId,
    ...
    async resolveConfig() {
        ...
        return {
            ...
            defaultOutputFormat: this.options.defaultOutputFormat ??
    ...
    async postJson(config, apiPath, payload) {
    ...
    async postForm(config, apiPath, form) {
        const response = await fetch(`${config.baseUrl}${apiPath}`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${config.apiKey}`
    ...
    async poll(config, taskId, timeoutMs, pollIntervalMs) {
        ...
        while (Date.now() < deadline) {
            ...
            if (isFailureStatus(status)) {
                await this.emit({
                    ...
                    status,
                    progress: taskProgress(lastResponse),
                    message: lastResponse.data?.fail_reason ??
                        lastResponse.fail_reason ??
                        lastResponse.data?.message ??
                        lastResponse.message
                });
                throw new Error(`NewAPI task ${taskId} failed: ${lastResponse.data?.fail_reason ??
                    lastResponse.fail_reason ??
                    lastResponse.data?.message ??
                    lastResponse.message ??
